...
def Stomata(a,t,parameters): 
    ...
    dAHA=prod-decay*AHA*(0.1+0.9*UBP24PRegulation)-phosph*AHA*(TOT3P2Regulation)+dephosph_basal*AHAP+dephosph_active*(AHAP*PP2CRegulation)
    dAHAP=-decay*AHAP*(0.1+0.9*UBP24PRegulation)+phosph*AHA*(TOT3P2Regulation)-dephosph_basal*AHAP-dephosph_active*(AHAP*PP2CRegulation) 
    ...
    dSLAC1=prod*SNRK2SRegulationSLAC1-decay*SLAC1 # Branch4
    return(dSNRK2S,dTOT3,dTOT3P1,dTOT3P2,dPP2C,dAHA,dAHAP,dUBP24,dUBP24P,dSLAC1)
...
for j in range(len(tempTest)):
    parameters = {'Km1':Km1,'Km2':Km2,'Km3':Km3,'Km4':Km4,'Km5':Km5,'aba': 0, 'temp': tempTest[j],'mutost1': 0,'noupb24':0,'notot3inact':0}  

    result = odeint(Stomata, IC, times, args=(parameters,))
    aha_output[j,idx,:] = result[-1,:]  
    stomata_aperture=(basal_stomata_aperture+result[-1, 6]**2/ (KmAHA**2 + result[-1, 6]**2)*(KmSLAC1**2/ (KmSLAC1**2 + result[-1, 9]**2)))*(Km5**2/(Km5**2+0**2))
...
cont=-1
for j in range(len(tempTest)):
    parameters = {'Km1':Km1,'Km2':Km2,'Km3':Km3,'Km4':Km4,'Km5':Km5,'aba': 0, 'temp': tempTest[j],'mutost1': 1,'noupb24':0,'notot3inact':0}
    result = odeint(Stomata, IC, times, args=(parameters,))
    aha_output[j,idx,:] = result[-1,:]  
    stomata_aperture=(basal_stomata_aperture+result[-1, 6]**2/ (KmAHA**2 + result[-1, 6]**2)*(KmSLAC1**2/ (KmSLAC1**2 + result[-1, 9]**2)))*(Km5**2/(Km5**2+0**2))
    if j==0 or j==len(tempTest)-1:
...
cont=-1
for j in range(len(tempTest)):
    parameters = {'Km1':Km1,'Km2':Km2,'Km3':Km3,'Km4':Km4,'Km5':Km5,'aba': 0, 'temp': tempTest[j],'mutost1': 0,'noupb24':1,'notot3inact':0}
    result = odeint(Stomata, IC, times, args=(parameters,))
    aha_output[j,idx,:] = result[-1,:]  
    stomata_aperture=(basal_stomata_aperture+result[-1, 6]**2/ (KmAHA**2 + result[-1, 6]**2)*(KmSLAC1**2/ (KmSLAC1**2 + result[-1, 9]**2)))*(Km5**2/(Km5**2+0**2))
    if j==0 or j==len(tempTest)-1:
...
cont=-1
for j in range(len(tempTest)):
    parameters = {'Km1':Km1,'Km2':Km2,'Km3':Km3,'Km4':Km4,'Km5':Km5,'aba': 0, 'temp': tempTest[j],'mutost1': 0,'noupb24':0,'notot3inact':1}
    result = odeint(Stomata, IC, times, args=(parameters,))
    aha_output[j,idx,:] = result[-1,:]  
    stomata_aperture=(basal_stomata_aperture+result[-1, 6]**2/ (KmAHA**2 + result[-1, 6]**2)*(KmSLAC1**2/ (KmSLAC1**2 + result[-1, 9]**2)))*(Km5**2/(Km5**2+0**2))
    if j==0 or j==len(tempTest)-1:
...
ax.set_xlim(-0.5, len(plottingvalues)-0.5)
ax.set_ylim(0, 1)
plt.tight_layout()
...
cont=-1
for abaidx in ABAvalues: # ABA 0 and 100 
    for tempidx in tempTest: # Temp 21 and 42 degrees C
        cont+=1
        parameters = {'Km1':Km1,'Km2':Km2,'Km3':Km3,'Km4':Km4,'Km5':Km5,'aba': abaidx, 'temp': tempidx,'mutost1': 0,'noupb24':0,'notot3inact':0}
        result = odeint(Stomata, IC, times, args=(parameters,))
...
plt.title('Stomata aperture under different conditions')
plt.ylim(0, 1)
plt.tight_layout()
